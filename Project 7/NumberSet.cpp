...
namespace cs31
{
	...
	{
		bool result = true;
		if (value == mOne || value == mTwo || value == mThree || value == mFour || value == mFive || value == mSix || value == mSeven || value == mEight) // check if the value is found in the box, column, or row
			{
				result = false ;
			}
		return(result);
