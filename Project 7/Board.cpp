...
namespace cs31
{
	...
	bool Board::completed() const
	{
		...
		for (int i = 1; i <= 9; i++)
		{
			for (int j = 1; j <= 9; j++)			//the two for loops check the whole game board
			{
				if (mCells[i][j].getValue() == 0)		// checks if the cell is a 0
				{
					result = false;
				}
				else
				{
					...
					NumberSet r = buildRow(i, j);
					NumberSet c = buildColumn(i, j);
					if (!mCells[i][j].isValueOK(mCells[i][j].getValue(), box, r, c))	// checks if the value within the cell is valid
					{
						result = false;
					}
	...
	int Board::evaluate() const
	{
		...
		int result = 0;
			for (int i = 1; i <= 9; i++)
			{
				for (int j = 1; j <= 9; j++)
				{
					if (mCells[i][j].getValue() != 0) // this function allows us to read every cell and if the cell does not equal an empty cell we add 1 to the counter
					{
						NumberSet box = buildBox(i, j);
						NumberSet r = buildRow(i, j);
						NumberSet c = buildColumn(i, j);
						if (mCells[i][j].isValueOK(mCells[i][j].getValue(), box, r, c))
						{
							result++ ;
						}
			...
			if (result == 81)	//if the board is fille we return a 100
			{
				result = 100;
			}
			return(result);
